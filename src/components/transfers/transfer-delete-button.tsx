...
export function TransferDeleteButton({ groupId }: { groupId: string }) {
  ...
  async function onDelete() {
    if (
      !confirm(
        "Delete this transfer? Both sides will be removed. This cannot be undone.",
      )
    ) {
      return;
    }
    setBusy(true);
    try {
      const res = await fetch(`/api/transfers/${groupId}`, {
        method: "DELETE",
      });
      if (!res.ok) {
